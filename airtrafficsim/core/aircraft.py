...
from airtrafficsim.utils.enums import APLateralMode, APThrottleMode, GroundPhase
from airtrafficsim.core.navigation import Nav
...
class Aircraft:
    ...
    def get_idx(self):
        idx_arr = np.where(self.traffic.index == self.index)[0]
        if len(idx_arr) == 0:
            return None
        return idx_arr[0]
    ...
    def is_taxiing(self):
        ...
        if idx is None:
            return False
        return self.traffic.ground_phase[idx] == GroundPhase.TAXI
    
    def start_taxi_to_runway(self, airport, runway):
        self.taxi_airport = airport

        lat = self.get_lat()
        lon = self.get_long()

        result = Nav.taxi_position_to_runway(
            airport,
            lat,
            lon,
            runway
        ...
        self.taxi_route = result["route"]
        self.taxi_hold_nodes = result["hold_short_nodes"]
        self.taxi_index = 0
        self.taxi_active = True

        # -----------------------------
        # TELEPORT TO FIRST TAXI NODE
        # -----------------------------
        if len(self.taxi_route) > 0:

            index = np.where(self.traffic.index == self.index)[0][0]

            first_lat, first_lon = self.taxi_route[0]

            self.traffic.lat[index] = first_lat
            self.traffic.long[index] = first_lon

            # Heading auf nächsten Taxi-Punkt ausrichten
            if len(self.taxi_route) > 1:

                next_lat, next_lon = self.taxi_route[1]

                bearing = Cal.cal_great_circle_bearing(
                    first_lat,
                    first_lon,
                    next_lat,
                    next_lon
                ...
                self.traffic.heading[index] = bearing
                self.traffic.ap.heading[index] = bearing

        self.taxi_index = 1
        from airtrafficsim.utils.enums import FlightPhase
        index = np.where(self.traffic.index == self.index)[0][0]
        self.traffic.flight_phase[index] = FlightPhase.TAXI_ORIGIN

    
    def update_taxi(self):
        if not self.taxi_active:
            return
        ...
        if self.taxi_index >= len(self.taxi_route):
            self.taxi_active = False
            self.traffic.taxi_speed[index] = 0
            return

        target_lat, target_lon = self.taxi_route[self.taxi_index]

        current_lat = self.get_lat()
        current_lon = self.get_long()
        ...
        current_heading = self.traffic.heading[index]

        diff = Cal.cal_angle_diff(current_heading, bearing)
        ...
        if dist < max(10, self.traffic.taxi_speed[index] * 2):
            self.taxi_index += 1
            return
        ...
        self.traffic.taxi_speed[index] = speed
        ...
        turn_rate = np.clip(turn_rate, -6, 6)

        self.traffic.heading[index] = current_heading + turn_rate
        self.traffic.bank_angle[index] = 0
