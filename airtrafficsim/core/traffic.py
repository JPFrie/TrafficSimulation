...
class Traffic:
    def __init__(self, file_name, start_time, end_time, weather_mode, performance_mode):
        ...
        # Ground Mode
        self.taxi_speed = np.zeros([0])
        self.ground_phase = np.zeros([0])
    ...
    def add_aircraft(self, call_sign, aircraft_type, flight_phase, configuration, lat, long, alt, heading, cas, fuel_weight, payload_weight, departure_airport, departure_runway, sid, arrival_airport, arrival_runway, star, approach, flight_plan, cruise_alt):
        ...
        # Ground Mode
        self.taxi_speed = np.append(self.taxi_speed, 0.0)

        if flight_phase in (FlightPhase.AT_GATE_ORIGIN, FlightPhase.AT_GATE_DEST):
            self.ground_phase = np.append(self.ground_phase, GroundPhase.PARKED)

        elif flight_phase in (FlightPhase.TAXI_ORIGIN, FlightPhase.TAXI_DEST):
            self.ground_phase = np.append(self.ground_phase, GroundPhase.TAXI)

        else:
            self.ground_phase = np.append(self.ground_phase, -1)
    ...
    def update_ground(self, d_t):
        taxi_mask = self.ground_phase == GroundPhase.TAXI
        parked_mask = self.ground_phase == GroundPhase.PARKED

        # parked aircraft
        self.taxi_speed[parked_mask] = 0
        self.gs_north[parked_mask] = 0
        self.gs_east[parked_mask] = 0

        speed = self.taxi_speed[taxi_mask]

        gs_north = speed * np.cos(np.deg2rad(self.heading[taxi_mask]))
        gs_east = speed * np.sin(np.deg2rad(self.heading[taxi_mask]))

        self.gs_north[taxi_mask] = gs_north
        self.gs_east[taxi_mask] = gs_east

        self.lat[taxi_mask] += gs_north * d_t / 216000
        self.long[taxi_mask] += gs_east * d_t / 216000
    ...
    def update(self, global_time, d_t=1):
        ...
        flight_phase_new = np.select(
            ...
            default=self.flight_phase
        )

        self.flight_phase[air_mask] = flight_phase_new[air_mask]

        # -----------------------------
        # Ground phase update
        # -----------------------------
        self.ground_phase = np.select(
            condlist=[
                (self.flight_phase == FlightPhase.AT_GATE_ORIGIN) |
        ...
        self.bank_angle = np.zeros_like(self.heading)

        self.bank_angle[air_mask] = np.select(
            condlist=[
                d_heading[air_mask] > 0.5,
                d_heading[air_mask] < -0.5
            ],
            choicelist=[
                self.perf.get_bank_angles(self.configuration[air_mask]),
                -self.perf.get_bank_angles(self.configuration[air_mask])
            ],
            default=0.0
        )
        ...
        # -----------------------------
        # Flight dynamics (aircraft in air only)
        # -----------------------------
        tas = Unit.kts2mps(self.tas)

        vs, accel = self.perf.cal_vs_accel(self, tas)

        self.vs[air_mask] = vs[air_mask]
        self.accel[air_mask] = accel[air_mask]

        tas[air_mask] = tas[air_mask] + self.accel[air_mask]

        mach = self.perf.tas_to_mach(tas, self.weather.T)
        cas = Unit.mps2kts(self.perf.tas_to_cas(tas, self.weather.p, self.weather.rho))

        self.mach[air_mask] = mach[air_mask]
        self.cas[air_mask] = cas[air_mask]

        self.tas[air_mask] = Unit.mps2kts(tas[air_mask])
        ...
        self.heading[air_mask] = heading_new[air_mask]
        ...
        # -----------------------------
        # Ground speed (aircraft in air)
        # -----------------------------
        gs_n = self.tas * np.cos(np.deg2rad(self.heading)) + self.weather.wind_north
        gs_e = self.tas * np.sin(np.deg2rad(self.heading)) + self.weather.wind_east

        self.gs_north[air_mask] = gs_n[air_mask]
        self.gs_east[air_mask] = gs_e[air_mask]
        ...
        # -----------------------------
        # Position update (aircraft in air)
        # -----------------------------
        self.lat[air_mask] += self.gs_north[air_mask] / 216000.0
        self.long[air_mask] += self.gs_east[air_mask] / 216000.0
        self.alt[air_mask] += self.vs[air_mask] / 60.0
